...
import { convertGLTFToFlowGraph } from "./Interactivity/FlowGraphConverter";
...
export class KHR_interactivity implements IGLTFLoaderExtension {
    ...
    public onReady(): void {
        ...
        const json = convertGLTFToFlowGraph(definition);

        const coordinator = FlowGraphCoordinator.Parse(json, scene, (key, serializationObject) => {});
        coordinator.start();
    }
}
